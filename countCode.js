...
objects = 0
// take every item from data one by one
// using that one item compare its first letter
function countCode(letter) {
    whileLoopCounter = 0
    let b = 0
    while (whileLoopCounter<data.length) { 
        if (data[whileLoopCounter].startsWith(letter)) {
            b++
        }
        whileLoopCounter++
    }
